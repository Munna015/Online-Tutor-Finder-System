<?php
session_start();

if(!isset($_SESSION['id']))
{
  header("location:index.php");
}


?>
<?php
include("confiq/db.php");
if(isset($_POST['Save']))
{
	$institution=$_POST["institution"];


	$uploadok=1;
	$file_name=$_FILES['Avatar']['name'];
	$file_size=$_FILES['Avatar']['size'];
	$file_tmp=$_FILES['Avatar']['tmp_name'];
	$file_type=$_FILES['Avatar']['type'];
	$target_dir="Tutor/uploads";
	$target_file=$target_dir.basename($_FILES['Avatar']['name']);
	$check=getimagesize($_FILES['Avatar']['tmp_name']);
	$text=explode('.',$_FILES['Avatar']['name']);
	$ext=end($text);
	$file_ext=strtolower($ext);

  /*$data = array(
   	'file_name' =>$file_name ,
   	'file_size' =>$file_size ,
   	'file_tmp' =>$file_tmp ,
   	'file_type' =>$file_type ,
   	'target_dir ' =>$target_dir,
   	'check' =>$check,
   	'file_ext' =>$file_ext

   

   );*/

  //print_r($data);



   $extensions=array("jpg","jpeg","png");
   if (in_array($file_ext, $extensions)==false) {
   	$error= "please choose the image which has the extensions (.jpg ,.jpeg ,.png )";
   }
   if($check==false)
   {
   	$error= "Sorry! the file is not image";
   }
   if(empty($error)==true)
   {
   	move_uploaded_file($file_tmp, "uploads/".$file_name);
   	$url = $_SERVER['HTTP_REFERER'];
   	$seg=explode('/', $url);
   	$path=$seg[0].'/'.$seg[1].'/'.$seg[2].'/'.$seg[3];
   	$full_url=$path.'/'."uploads/".$file_name;
    $id=$_SESSION['id'];
   
   // sql to delete a record
    $sql1 = "DELETE FROM profile WHERE user_rule='$id'";
   if( $conn->query($sql1) === TRUE)
   {
	$sql = "INSERT INTO profile(Institution,Avatar,user_rule) VALUES ('$institution','$full_url','$id')";
   	if ($conn->query($sql) === TRUE) {
   		$error ="Saved Seccessfully!";
   	} else {
   		$error= "Failed to Upload Image!";
   	}
   	
   }
   else{
   	$error= "Oops! Something went wrong.";
   }
}
   $conn->close();

}


?>

<!DOCTYPE html>
<html>

<head>

	<meta charset="utf-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<link href='css/Header.css' rel='stylesheet' type='text/css'>
	<link href='css/Footer.css' rel='stylesheet' type='text/css'>
	<link href='css/User.css' rel='stylesheet' type='text/css'>
	<link href='css/Avatar.css' rel='stylesheet' type='text/css'>
	<link href='css/Profile.css' rel='stylesheet' type='text/css'>
	<link href='css/alert.css' rel='stylesheet' type='text/css'>

	
	<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">


	<link href='https://fonts.googleapis.com/css?family=Cookie' rel='stylesheet' type='text/css'>
	



</head>



<body>
	<div class="header">
		<a href="index.php" class="logo">Tution<span>Media</span></a>
		<div class="header-right">
			<a  href="index.php">Home</a>
			<a href="tutor.php">Tutors</a>
		</div>
	</div>
	<?php if(isset($_POST['Save'])): ?>
		<div style="background-color: #5eb762;color: white;"   class="alert">
			<span class="closebtn" onclick="this.parentElement.style.display='none';">&times;</span> 
			<strong> <?php echo $error;?></strong>
		</div>
	
<?php endif; ?>

<h4 style="float: right;margin-right: 30px;">Welcome  <?php echo $_SESSION['Username'];?></h4>


<div class="row">
	<div class="column left">
		<table>
			<tr >
				<td><a href="User.php">Dashboard</a></td>

			</tr>
			<tr >
				<td><a href="Profile.php"> Add Profile</a></td>

			</tr>
			<tr>
				<td><a href="Post.php">Create Post</a></td>

			</tr>
			<tr>
            <td><a href="youpost.php">Your Post</a></td>

            </tr>
			<tr >
				<td><a href="Logout.php">Logout</a></td>

			</tr>
		</table>

	</div>
	<div class="column right" >



		<div>
			<form action="Profile.php" method="POST" enctype="multipart/form-data">

				<h2 style="font-size: 30px;"><b>Add Profile</b></h2>
				<hr>
				<div class="profile-font">
					<label for="institution" ><b>Institution</b></label><br>
					<input type="text" placeholder="Enter Institution Name" name="institution" id="institution" required ><br>



					<label for="Avatar"><b>Change profile Picture</b></label><br>
					<input type="file" placeholder="Avatar" name="Avatar" id="Avatar" required >

				</div>
				<hr>


				<button type="Submit" name="Save" class="savebtn"  >Save</button>

				<button type="reset" name="Cancel" class="savebtn" >Cancel</button>
			</form>
			</div>

		</div>
	</div>

	<div>

		<?php
		include 'inc/Footer.inc.php';
		?>
	</div>

</body>
</head>
</html>
